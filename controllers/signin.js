  ...
  try {
    sendEmail(
      ...
      (html =
        "<p>Hi " +
        name +
        ',Please copy the link and <a href="http://localhost:8000/reset-password?id=' +
        id +
...
const updatePassword = async (req, res) => {
  try {
    ...
    if (!id || !password) {
      return res
        .status(400)
        .json({ success: false, msg: "User ID and password are required" });
    }
    ...
    if (data) {
      ...
      res
        .status(200)
        .json({ success: true, msg: "Your password has been updated" });
    } else {
      res.status(404).json({ success: true, msg: "User Id not found" });
    }
  } catch (err) {
    res.status(400).json(err.message);
  }
